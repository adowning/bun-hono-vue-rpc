...
export const authMiddleware = () =>
  createMiddleware<AppEnv>(async (c, next) => {
    ...
    try {
      ...
      // || c.req.query('token') || c.req.query('access_token')
      console.log('bear ', accessToken)
      console.log('bearx')
      console.log(c.req.queries())
      console.log(c.req.query('access_token'))
      ...
      accessToken = accessToken?.split('&wa')[0]
      ...
      if (tokenParts.length === 3) {
        cacheKey = tokenParts[2] || null// Use the signature
      } else {
