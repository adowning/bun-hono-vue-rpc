...
if (typeof window.nge === 'undefined') window.nge = {}
...
nge.Common.Lib.Helper = {
        ...
        $('img.autoFix').error(function () {
          ...
          setTimeout(
            function () {
              ...
              var newSrc =
                (src.indexOf('?') !== -1 ? src.substr(0, src.indexOf('?')) : src) +
                '?t=' +
                new Date().getTime()
  ...
  infrequentHandler: function (func, delay) {
    ...
    return function () {
      ...
      if (functionTimer) clearTimeout(functionTimer)

      functionTimer = setTimeout(function () {
        functionTimer = false
  ...
  setObjKeyInPosition: function (key, keyIndex, value, object) {
    ...
    for (var i = 0; i < keys.length; i++) {
      var k = keys[i]
      var val = keyIndex === i ? value : object[k]
      delete object[k]
      object[k] = val
    }
    ...
    for (i = 0; i < elements.length; i++) {
      ...
      switch (newSelector.slice(0, 1)) {
        ...
        case '.':
          ...
          } else classArrN.push(newSelector.slice(1))

          model.class = classArrN.join(' ')
          break
        default:
          console.error(
            'nge.Common.Lib.Helper.objectClone newSelector error',
            selector,
            newSelector
          )
      }
  ...
  conditionCheck: function (condition, callback, delay, maxCycles, callAnywayFlag) {
    if (typeof maxCycles === 'undefined') maxCycles = 20
    ...
    if (maxCycles > 1) {
      var clbk = function () {
        nge.Common.Lib.Helper.conditionCheck(
          condition,
          callback,
          delay,
          maxCycles - 1,
  ...
  makeReactive: function (obj, key, callback) {
    ...
    if (!descriptor)
      // bad descriptors and its IOS mobile
      return false
...
nge.setStatisticsHandlers = function () {
  ...
  nge.observer.add(
    ...
    'statisticsCollectorOneTimeHandler',
    true
  )
...
;(function () {
  ...
  var Fullscreen = function () {
    ...
    var _isIphone5 =
      device.pixelRatio === 2 &&
      device.iPhone &&
      ((screen.availWidth === 320 && screen.availHeight === 568) ||
        (screen.availWidth === 568 && screen.availHeight === 320))
    ...
    if (device.iPhone) {
      window.addEventListener(
        ...
        function (e) {
          ...
          var stopMenuTouchMove =
            e.target.className.indexOf('menu') !== -1 ||
            e.target.className.indexOf('sidebar') !== -1

          if (stopMenuTouchMove) e.preventDefault()

          if (nge && nge.localData && nge.localData.get('allowTouchmove') && !multiplyTouches)
            return false

          if (!actionNeeded || multiplyTouches || scaleNotOne || _isIphone5) e.preventDefault()
        },
        { passive: false }
      )
    ...
    var _checkWindowFullscreen = function () {
      if (typeof document.isFullScreen !== 'undefined') return document.isFullScreen
    ...
    var _switchFullscreenMessageVisible = function (visible) {
      ...
      if (_fullscreenMessageLayer) {
        if (_getDevice().iOS && _isUiWebView) display = 'hidden'
    ...
    this.windowOrientationCheck = function () {
      if (_isFullscreen && !_checkWindowFullscreen()) nge.observer.fire('fullscreen.off')
    ...
    var _forceWindowResizeIosSafari = function () {
      if (_isChromeIOS()) {
        if (_getDevice().iPad) $('body').css({ width: window.outerWidth })
        return
      }
      ...
      var hideByHeight =
        $(window).height() === window.innerHeight || (isPortraitMode && _isAdaptive)
      ...
      var hideHand = hideByHeight && $(window).width() === window.innerWidth
      var y = document.body.getBoundingClientRect().y

      if (y !== 0 && !_touchInProgress) window.scroll(0, 0)
      ...
      var initializeFullscreen = function () {
        ...
        var onHTMLLoaded = function (html) {
          ...
          $(window).on('resize', function () {
            if (isPortrait()) return _switchFullscreenMessageVisible(false)
        ...
        var onStartScreenLoaded = function (html) {
          ...
          } else _fullscreenMessageLayer.style.display = 'block'
  ...
  var ScreenOrientation = function () {
    ...
    this.windowOrientationCheck = function () {
      if (!_orientCtx || !_orientCtx.posWarningLayer || !_orientCtx.posWarningLayer.style) return
      ...
      var block =
        (portrait && availableOrientations.indexOf('portrait') === -1) ||
        (!portrait && availableOrientations.indexOf('album') === -1)
      _orientCtx.posWarningLayer.style.display = block ? 'block' : 'none'
    }

    this.init = function () {
      var ios = !!navigator.platform && /iPad|iPhone|iPod/.test(navigator.platform)
      nge.Common.Lib.Helper.getHtml('../../tpl/posWarning/poswarningIOS.html', function (html) {
        html = _localizationApply(html)
        $('body').append(html)
  ...
  var _windowOrientationCheck = function () {
    if (_orientationController && _readyToPlay) _orientationController.windowOrientationCheck()
    if (_fullscreenController) _fullscreenController.windowOrientationCheck()
  }
  ...
  var _localizationApply = function (str) {
    ...
    if (!lang || !_globalTranslations[lang]) return str
  ...
  $(document).on('loaderTplAppended', function () {
    ...
    if (reloadButton) reloadButton.innerHTML = _localizationApply(reloadButton.innerHTML)
    ...
    if (
      ...
      if (!mobile) _showLoaderMessage(_localizationApply('{{mobileVersionRecommended}}'))
    }
  })
  ...
  var _globalTranslations = {
    en: {
      ...
      mobileVersionRecommended:
        'For best experience on this device,<br>we recommend to use mobile version of browser.'
    },
    ru: {
      ...
      mobileVersionRecommended:
        'Для лучшей работы на данном устройстве<br>мы рекомендуем использовать мобильную версию браузера.'
    },
    de: {
      ...
      mobileVersionRecommended:
        'Für die beste Erfahrung auf diesem Gerät empfehlen<br>wir die Verwendung der mobilen Version des Browsers.'
    },
    es: {
      TouchToStart: 'Pulsa para continuar',
      PleaseTurnYourDeviceToLandscapeMode:
        '¡Por favor, convierte tu dispositivo en modo horizontal!',
      reloadGame: 'RECARGAR EL JUEGO',
      mobileVersionRecommended:
        'Para obtener la mejor experiencia en este dispositivo,<br>recomendamos que utilices la versión móvil del navegador.'
    },
    tr: {
      ...
      mobileVersionRecommended:
        'Bu aygıtta en iyi deneyimi sağlamak için,<br>tarayıcınızın mobil versiyonunu kullanmanızı tavsiye ederiz.'
    },
    it: {
      ...
      mobileVersionRecommended:
        'Per una migliore esperienza su questo dispositivo,<br>consigliamo di utilizzare la versione mobile del browser.'
    },
    fr: {
      ...
      mobileVersionRecommended:
        "Pour profiter au mieux de votre expérience sur cet appareil,<br>nous vous recommandons d'utiliser la version mobile de votre navigateur."
    },
    pt: {
      ...
      mobileVersionRecommended:
        'Para uma melhor experiência neste dispositivo,<br>recomendamos que use a versão móvel do navegador.'
    },
    ar: {
      ...
      mobileVersionRecommended:
        'نوصي باستخدام نسخة المتصفح للهاتف النقال<br>لأفضل تجربة على هذا الجهاز'
    },
    zh: {
      TouchToStart: '开始触摸',
    ...
    ja: {
      ...
      mobileVersionRecommended:
        'この機器で最高の体験をするにはモバイルバ<br>ージョンのブラウザを使用する事をお勧めします。'
    },
    th: {
      ...
      mobileVersionRecommended:
        'เราขอแนะนำให้ใช้งานเบราว์เซอร์ที่เป็นเวอร์ชั่นมือถื<br>อเพื่อการได้รับประสบการณ์ที่ดีที่สุดบนอุปกรณ์ชิ้นนี้'
    },
    id: {
      ...
      mobileVersionRecommended:
        'Untuk pengalaman terbaik di perangkat ini,<br>kami sarankan menggunakan peramban versi ponsel.'
    },
    ms: {
      ...
      mobileVersionRecommended:
        'Untuk pengalaman terbaik pada peranti ini,<br>kami syorkan untuk guna pelayar versi mudah alih.'
    },
    vi: {
      TouchToStart: 'Chạm để bắt đầu',
      PleaseTurnYourDeviceToLandscapeMode:
        'Vui lòng chuyển thiết bị của bạn sang chế độ cảnh quan!',
      reloadGame: 'TẢI LẠI TRÒ CHƠI',
      mobileVersionRecommended:
        'Bạn nên sử dụng phiên bản di động<br>để có được trải nghiệm tốt nhất trên thiết bị.'
    },
    hi: {
      ...
      mobileVersionRecommended:
        'इस उपकरण पर अच्छे अनुभव के लिए,<br>हम आपको ब्राउज़र के मोबाइल संस्करण को इस्तेमाल करने की सिफारिश करते हैं।'
    },
    ko: {
      ...
      reloadGame: '게임 재시작',
      mobileVersionRecommended:
        '해당 디바이스에서 최고의 경험을 위해,<br>모바일 버전 브라우저 사용을 권장합니다.'
    },
    sv: {
      ...
      mobileVersionRecommended:
        'För bästa upplevelse på den här enheten rekommenderar<br>vi att du använder den mobila versionen av webbläsaren.'
    }
  }
})()
...
nge.statisticsCollector = new (function () {
  ...
  var _apiUrl = '/game/AfricanKingNG/server?sessionId=' + accessToken
  var _sesId = false
  var _query = []
  var _lastSendCall

  this.init = function () {
    if (_apiUrl.indexOf('URL_STATISTICS_COLLECTOR') === 1) _apiUrl = false
  ...
  var _store = function (data, dontSendNowFlag) {
    ...
    if (!dontSendNowFlag) _chkStorage()
  ...
  var _chkStorage = function () {
    ...
    if (_query.length === 1) _sendSingle()
    else _sendAll()
  }
  ...
  var _receive = function (data) {
    ...
    if (data && data.sesId) _sesId = data.sesId
  }
})()
...
nge.HotPatches.patches = {
  ...
  ipadOsPhaserFix: {
    1: function () {
      ...
      if (
        ...
        typeof Phaser.VERSION !== 'undefined' &&
        nge.game.device.macOS &&
        navigator.maxTouchPoints &&
        navigator.maxTouchPoints > 1
      ) {
        ...
        if (uam[2] >= 77)
          if (typeof Phaser !== 'undefined' && typeof Phaser.VERSION !== 'undefined') {
            ...
            for (var i = 0; i < smTh.length; i++)
              if (!blursCrutch && smTh[i] > 15) {
                // blursCrutch flag garants one time call
                var newThs = []
                var addBlurs = []
                ...
                for (var j = 0; j < smTh.length; j++) {
                  ...
                  if (smMh <= 3) {
                    //tempBlurs have just 3 symbols
                    var addBlursReel = ~~((smTh[j] - 3) / 3)
                    newTh = smTh[j] - addBlursReel * 3

                    addBlurs.push(addBlursReel)
                  } else newTh = ~~(smMh + (j * (15 - smMh)) / (smTh.length - 1)) //smMh-15 size of blurs
    ...
    1: function () {
      ...
      var paidJackpotsLogic = function () {
        ...
        nge.brain._logicBlocksInstances.game.updateBalance = function () {
          ...
          if (amountWithoutJackpot < 0) return false
          ...
          if (!result) nge.localData.set('balance.totalAmount', totalAmount)
      ...
      nge.observer.add(
        'transportMessage.JackpotResponse',
        function (data) {
          if (paidJackpots) return true

          var jrData = data.data

          if (
            (jrData.jackpotPaid === true || jrData.jackpotPaid === 'true') &&
            +jrData.jackpotCost > 0
          ) {
  ...
  instantJackpots: {
    1: function () {
      ...
      var authResponseModelHardcode = function (d) {
        return {
          ...
          data: {
            ...
            gameParameters: {
              availableLines: nge.Lib.Helper.recursiveGet(
                'data.gameParameters.availableLines',
                d,
                []
              ),
              payouts: nge.Lib.Helper.recursiveGet('data.gameParameters.payouts', d, []),
              initialSymbols: nge.Lib.Helper.recursiveGet(
                'data.gameParameters.initialSymbols',
                d,
                []
              ),
              rtp: nge.Lib.Helper.recursiveGet('data.gameParameters.rtp', d, false)
            },
            supportedFeatures: nge.Lib.Helper.recursiveGet(
              'data.gameParameters.supportedFeatures',
              d,
              []
            ),
      ...
      var AuthResponseModelPath = nge.App.getPath(
        'Mlm.Transport.Models.AuthResponse',
        false,
        'instantJackpots'
      )
      ...
      nge.observer.add(
        'transportMessage.AuthResponse',
        function (data) {
          if (instantJackpotsMode) return true
      ...
      var updateBalance = function () {
        ...
        if (nge.localData.get('uiType') === 'gold') {
          //if in game no ui_v3 instance modes
        ...
        if (isSweepStakes) {
          var text =
            nge.Lib.Money.toCoinsInt(data.totalAmount) +
            ' / ' +
          ...
          for (var k in creditsAmounts) {
            ...
            if (creditsAmounts[k].maxSizeResize) creditsAmounts[k].maxSizeResize('width', maxWidth)
          }

          for (var k in balanceNumbers) {
            balanceNumbers[k].text = text
            if (balanceNumbers[k].maxSizeResize) balanceNumbers[k].maxSizeResize('width', maxWidth)
          }
          return
        }
      ...
      var instantJackpotsPopupLogic = function () {
        var jackpotPopupPreload = function (state) {
          if (state !== 'play') {
            // adding instant jackpot assets to load in play state preload
            ...
            cbContainer.contents = [
              new nge.Mlm.Assets.Image({
                key: 'jackpotPopupCustomName1',
                block: 1,
                fullPath: 'img/jackpots/names/popup_jackpot_1.png'
              }),
              new nge.Mlm.Assets.Image({
                key: 'jackpotPopupCustomName2',
                block: 1,
                fullPath: 'img/jackpots/names/popup_jackpot_2.png'
              }),
              new nge.Mlm.Assets.Image({
                key: 'jackpotPopupCustomName3',
                block: 1,
                fullPath: 'img/jackpots/names/popup_jackpot_3.png'
              }),
              new nge.Mlm.Assets.Image({
                key: 'jackpotPopupCustomName4',
                block: 1,
                fullPath: 'img/jackpots/names/popup_jackpot_4.png'
              })
            ]
          ...
          if (nge.wrap && nge.wrap.load) {
            nge.wrap.load.font(
              'jackpotsCustomFuturasbl',
              nge.realPathPrefix + 'css/fonts/jackpots/custom/futurasbl.otf' + antiCacheSuffix
            )
            nge.wrap.load.start()
          } else {
            nge.game.load.font(
            ...
            nge.game.load.onLoadComplete.addOnce(function (a) {
              ...
              style.innerHTML =
                ...
                'src: ' +
                "url('" +
                nge.game.cache.getFont('jackpotsCustomFuturasbl') +
                "') format('opentype');" +
                '}'
        ...
        var spaceHandlerController = nge.App.getPath(
          'Com.SpaceHandler.Controller',
          false,
          'spaceHandler'
        )

        if (spaceHandlerController) {
          nge.App[nge.appNS].Com.SpaceHandler.Controller = spaceHandlerController.extend(
            function () {
        ...
        var showPopup = function () {
          ...
          if (typeof popupInitialY === 'undefined') popupInitialY = jackpotPopupInnerContainer.y

          jackpotPopupInnerContainer.y = popupInitialY

          var newTween = nge.tween
            ? nge.tween.add.bind(nge.tween)
            : nge.game.add.tween.bind(nge.game.add)
          ...
          showPopupTween.onStart.add(function () {
            ...
            var engineVersion =
              typeof Phaser !== 'undefined' && typeof Phaser.VERSION !== 'undefined' ? 1 : 2
            var mainSprite

            //close callback
            var jackpotClose = function () {
              if (!showPopupTime) return false

              var closeWaitTime = 1000 //1sec

              if (nge.Lib.Time.get() - showPopupTime < closeWaitTime) return false
              ...
              hidePopupTween.onComplete.add(function () {
                ...
                if (
                  ...
                  statusContainer.parent.getData &&
              ...
              if (mainSprite.events) mainSprite.events.onInputDown.removeAll()
            ...
            //close handler
            if (engineVersion === 1) {
              mainSprite = nge.findAll('^jackpotPopup').find(function (el) {
                return el instanceof Phaser.Sprite
              })
        ...
        nge.observer.add(
          'StatesManager.create.end',
          function (state) {
            if (state === 'play' && savedData) showPopup()
          },
          false,
          true
        )
        ...
        if (
          ...
          !nge.App.DjGameBase &&
          !nge.App.RiverGameBase
        ) {
          nge.observer.add(
            'autospinSimple.on',
            function () {
              if (jackpotWinned) nge.observer.fire('gamble.take')
            },
            false,
            true
          )
          nge.observer.add(
            'slotMachine.spinStart',
            function () {
              jackpotWinned = false
            },
            false,
        ...
        nge.observer.add(
          'transportMessage.JackpotPlayResponse',
          function (data) {
            if (!instantJackpotsMode) return false
            ...
            if (nge.statesManager.getCurrentName() === 'play') showPopup() //show popup
          },
          false,
          true
        )
  ...
  sweepsStakes: {
    1: function () {
      ...
      var modes = nge.App.getSysInstancesModes
        ? nge.App.getSysInstancesModes()
        : nge.localData.get('_sysInstancesModes') || []
      var mode = 'Gold'
      var color = 'gold'
      ...
      nge.observer.add(
        'transportMessage.AuthResponse',
        function (data) {
          if (sweepsStakesMode) return true
      ...
      var sweepsBalance = function () {
        ...
        var balanceUpdateHandler = nge.Common.Lib.Helper.infrequentHandler(function (d) {
          balanceToSweeps(d)
        }, delayForLockedCalls)
        ...
        var balanceToSweeps = function (bData) {
          ...
          if (bData && bData.totalAmount) data.totalAmount = bData.totalAmount

          if (bData && bData.totalAmountReal) data.totalAmountReal = bData.totalAmountReal
          ...
          if (balanceNumbers)
            for (i = 0; i < balanceNumbers.length; i++) bcNumbers.push(balanceNumbers[i])

          if (creditsAmounts)
            for (i = 0; i < creditsAmounts.length; i++) bcNumbers.push(creditsAmounts[i])
        ...
        var winFieldUpdateHandler = nge.Common.Lib.Helper.infrequentHandler(function (d) {
          setWinFieldValue(d)
        }, delayForLockedCalls)
        ...
        var setWinFieldValue = function (value) {
          ...
          if (!winNumbers) winNumbers = []
          var winNumbersCredits = nge.findAll('.creditsWinNumberClone')
          if (!winNumbersCredits) winNumbersCredits = []
          ...
          if (value && typeof value === 'number') {
            ...
            newText =
              typeof value === 'undefined' || !value
                ? nge.i18n.get(wonWinText)
                : nge.i18n.get(value)
          }

          for (var i = 0; i < allWinNumbers.length; i++) {
            if (typeof newText !== 'undefined' && newText.toString) allWinNumbers[i].text = newText
          }
        }
        ...
        var balanceAmountInDjRiverGames = function (data) {
          if (data && !data.entries) balanceUpdateHandler(data)
        }
        ...
        if (nge.App.RiverGameBase) {
          ...
          var subscribesForSlowBalance = function () {
            //Thunder Strike

            nge.observer.add('StatesManager.create.end', function (state) {
              var gamesList = ['121', '146', '235', '254', '256']
              if (state === 'play' && gamesList.indexOf(nge.gameCode) !== -1) balanceUpdateHandler()
            })

            nge.observer.add(
              'entries.amount',
              balanceUpdateHandler,
              'entriesAmountOldPrototypes',
              true
            )
            nge.observer.add(
              'balance.amount',
              balanceUpdateHandler,
              'balanceAmountOldPrototypes',
              true
            )
            nge.observer.add('win', balanceUpdateHandler, 'winOldPrototypes', true)
          }

          var subscribesForFastBalance = function () {
            //Cleos Heart, wolf reels
            nge.observer.add(
              'entries.amount',
              entriesAmountInDjRiverGames,
              'entriesAmountDjRiverGames',
              true
            )
            nge.observer.add(
              'balance.amount',
              balanceAmountInDjRiverGames,
              'balanceAmountDjRiverGames',
              true
            )
            nge.observer.add(
              'transportMessage.BuyTimeResponse',
              function () {
                buyTimeResponseReceived = true
              },
              'buyTimeNewPrototypes',
              true
            )
            nge.observer.add(
              'Transport.close',
          ...
          var path = isMobileOrTablet
            ? 'Com.Balance.Mobile' + addition + '.Controller'
            : 'Com.Balance.UI_v3' + modePath + '.Controller'
          var balanceController = nge.App.getPath(path, false, 'sweepsBalanceField')
          if (balanceController) {
            nge.Lib.Helper.recursiveSet(
              'App.' + nge.appNS + '.' + path,
              balanceController.extend(function () {
                if (['257', '258'].indexOf(nge.gameCode) !== -1) this.currencyColor = '#EFC273'
        ...
        if (nge.App.DjGameBase) {
          ...
          if (typeof Phaser !== 'undefined' && typeof Phaser.VERSION !== 'undefined') {
            ...
            nge.observer.add(
              'StatesManager.create.end',
              function (state) {
                if (state === 'jackpot') currentDjRiverBalance = null
              },
              'statesCreateDj',
              true
            )
        ...
        function moveWinUpOnFreespins() {
          ...
          function addDelta(delta) {
            ...
            if (winNumber) winNumber.y += delta

            var winNumberCredits = nge.findOne('.creditsWinNumberClone')
            if (winNumberCredits) winNumberCredits.y += delta
          }
      ...
      var cashSweepsStakesPopup = function () {
        ...
        var sendPopup = function (e) {
          if (_buyTimeResponseWaiting) return
          ...
          if (amount === 'all') amount = 0 //amount:0 if buyAll
        ...
        var cashCreate = function (state) {
          if (state !== 'play' && state !== 'pickBonus' && state !== 'bonusWheel') return false

          var blockBuyTimeButton = nge.Lib.Helper.parseGetParams('blockBuyTimeButton')

          if (blockBuyTimeButton) return false

          var buttonContainer =
            nge.findOne('^UIBottomPanelsContainer') ||
          ...
          if (mobile) {
            ...
            if (color === 'blue') {
              ...
              textX = 84
              textY = 97
            } else if (nge.gameCode === '290') {
              // Volcano Fruits
              textStyleMobile.fill = '#ffffff'
              x = 1683
              y = 640
              textX = 91
            } else if (nge.gameCode === '261') {
              // Fruits Fury
              textStyleMobile.fill = '#ffffff'
              x = 1702
              y = 640
            ...
            if (nge.Cfg.Main.gameCode === '216') {
              ...
              if (c && c.parent) c.parent.bringToTop(c)
            }
          } else {
            var textStyle = {
        ...
        var cashButtonLogic = function () {
          if (_cashButtonLogicSetupped) return false

          //visibility logic
          if (!mobile)
            //no logic for desktop
            return false
        ...
        var hidePopup = function (popupName) {
          ...
          if (btn && popupName === 'insufficientFundsPopup') btn.visible = true
        }
  ...
  totalBetSharedLogic: {
    1: function () {
      ...
      var totalBetUpdate = function () {
        ...
        if (typeof extraBet === 'object')
          // new
          extraBetValue = extraBet.value ? extraBet.multiplier : 1 // old
        else extraBetValue = extraBet === '1' ? nge.localData.get('extraBetMult') : 1

        if (nge.gameCode === '139')
          // for Book Of Ra Deluxe 6 there is no multiplier
  ...
  customButtonsDefaultsCheck: {
    1: function () {
      ...
      var customButtonCheck = function (param, buttonName, imageKey) {
        ...
        if (!window.customButtons) window.customButtons = {}
  ...
  jackpotsAndSpinConflict: {
    1: function () {
      ...
      nge.brain._logicBlocksInstances.slot.customCheckCanSpin = function () {
        if (!customCheckCanSpinOriginal()) return false

        if (nge.localData.get('jackpot.canPlay')) {
          var freespinInProgress = nge.localData.get('freespin.inProgress')
          var respinInProgress = nge.localData.get('respin.inProgress')

          if (!freespinInProgress && !respinInProgress) return false
        }
  ...
  macOsX11HelperGetOs: {
    _condition: function () {
      return typeof nge.Lib !== 'undefined' && typeof nge.Lib.Helper !== 'undefined'
    },
    1: function () {
  ...
  iOS14PreserveDrawingBufferDisabling: {
    ...
    1: function () {
      ...
      if (!isIos14 || isPhaser) return false
      ...
      var ctor = function () {
        ...
        this.init = function (config) {
          if (_self.renderer) return _self
          ...
          _self.renderer = new PIXI.WebGLRenderer({
            width: config.width,
            height: config.height,
            antialias: config.antialias
          }) //todo: check if it is correct? if no - we shell use our renderer param
          ...
          _self.interaction = new PIXI.interaction.InteractionManager({
            root: _root,
            view: _self.renderer.view
          })
        ...
        this.loop = function () {
          if (_loopStopped) return false

          _requestAnimFrame(function () {
            _self.loop()
          })

          if (!_loopPaused) _self.update()

          return true
        }

        this.update = function () {
          if (!_self.currentState || _self.currentState.isPaused()) return
        ...
        this.add = function (id, stateObject) {
          if (_self.states[id]) return false
        ...
        this.start = function (id, clearWorld) {
          if (_self.states[id]) {
            if (_self.currentState) _self.currentState.pause()
        ...
        this._rescale = function () {
          _self.ratio = Math.min(
            window.innerWidth / _originalWidth,
            window.innerHeight / _originalHeight
          )
        ...
        this.findStaticContainers = function () {
          if (!nge.Lib.Helper.customRecursiveFind) return []
        ...
        this.createStaticContainers = function (staticContainers) {
          if (!staticContainers || staticContainers.length === 0) return false

          var staticContainer = nge.objects.create(
            nge.Mlm.Objects.Folder({
  ...
  cookiesCrossDomainAndExpireFix: {
    ...
    1: function () {
      nge.Lib.Helper.setCookie = function (c_name, value, exdays) {
        if (typeof exdays === 'undefined') exdays = 30 * 12 //one year
    ...
    1: function () {
      ...
      nge.Mlm.StatesManager.Controller = baseStatesManagerController.extend(function () {
        this.display = function (state, changeMode, clearWorld) {
          if (
            state === 'load' &&
            (nge.App.DjGameBase || nge.App.RiverGameBase || nge.gameCode === '277')
          ) {
            var pathJackpot = 'Com.Jackpot.Controller'
            var jackpotController = nge.App.getPath(
              'Com.Jackpot.Controller',
              false,
              'jackpotController'
            )
              ...
              nge.Lib.Helper.recursiveSet(
                ...
                jackpotController.extend(function () {
                  this.initLayers = function () {
                    ...
                    jackpotPopupWinText = nge.objects.create(
                      textModel,
                      jackpotPopupWinContainer,
                      true
                    )
            ...
            if (loadCfg) {
              nge.Lib.Helper.recursiveSet(
                ...
                loadCfg.extend(function () {
                  ...
                  var heavyPath =
                    nge.realPathPrefix +
                    'games/AfricanKingNG/css/fonts/FuturaPT-Heavy.otf' +
                    antiCacheSuffix
                  var mediumPath =
                    nge.realPathPrefix +
                    'games/AfricanKingNG/css/fonts/FuturaPT-Medium.otf' +
                    antiCacheSuffix
                  ...
                  for (var fontKey in this.fonts) {
                    ...
                    if (fontPathLower.indexOf('futura') !== -1) {
                      if (fontPathLower.indexOf('heavy') !== -1) this.fonts[fontKey] = heavyPath
                      if (fontPathLower.indexOf('medium') !== -1) this.fonts[fontKey] = mediumPath
                    }
                  }
                }),
                nge
    ...
    1: function () {
      ...
      var changeTotalBetText = function () {
        selectorsToTextReplace.forEach(function (selector) {
          ...
          if (obj) obj.text = TEXT_TO_REPLACE
        })
      }
...
nge.HotPatches.init = function () {
  if (nge.HotPatches.done) return false

  //all is loaded, lets start
  var doneFlag = true
  var patchesCfg =
    nge.App && nge.App.getInstance && nge.App.getInstance('Cfg.HotPatches')
      ? nge.App.getInstance('Cfg.HotPatches').get()
      : {}

  //lets go check our nge.HotPatches
  for (var patchName in nge.HotPatches.patches) {
    if (!patchesCfg[patchName]) patchesCfg[patchName] = 0
    ...
    for (var version in nge.HotPatches.patches[patchName]) {
      if (version === '_condition') continue

      if (patchesCfg[patchName] < version) {
        if (!nge.HotPatches.patches[patchName]._done) nge.HotPatches.patches[patchName][version]()
...
;(function () {
  ...
  nge.Common.Lib.Helper.makeReactive(window.nge, 'phaserAdapter', function () {
    nge.HotPatches.init()
  })
