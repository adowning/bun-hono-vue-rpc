...
import type { AckConfig } from './utils/acknowledgments'
import type { BatchConfig } from './utils/batch-operations'
import type { EncryptionConfig } from './utils/encryption'
import type { LoadConfig } from './utils/load-management'
import type { AuthConfig, RateLimitConfig, SecurityConfig } from './utils/middleware'
import type { PersistenceConfig } from './utils/persistence'
import type { HeartbeatConfig } from './utils/presence-heartbeat'
import type { RedisConfig } from './utils/redis-adapter'
import type {
  BroadcastConfig,
  ClientEventMessage,
  NetGameMessage,
...
import type { WebhookConfig } from './utils/webhooks'
import process from 'node:process'
import { AcknowledgmentManager } from './utils/acknowledgments'
import { BatchOperationsManager } from './utils/batch-operations'
import { Broadcaster } from './utils/broadcaster'
import { ChannelNamespaceManager, ChannelStateManager } from './utils/channel-state'
import { ChannelManager } from './utils/channels'
import { EncryptionManager } from './utils/encryption'
import { BroadcastHelpers } from './utils/helpers'
import { ChannelLifecycleManager } from './utils/lifecycle-hooks'
import { LoadManager } from './utils/load-management'
import {
...
import { PresenceHeartbeatManager } from './utils/presence-heartbeat'
import { RedisAdapter } from './utils/redis-adapter'
import { WebhookManager } from './utils/webhooks'
import { AuthenticationManager } from './middleware/auth.middleware'
import { handleNetGameMessage } from './handlers/netgame.handler'
...
export class BroadcastServer {
   ...
   */
  async start(server: BunServer<WebSocketData>): Promise<void> {
  ...
  public async handleMessage(ws: ServerWebSocket<WebSocketData>, message: string | Buffer): Promise<void> {
    ...
    if (typeof message === 'string' && message.startsWith(':::')) {
      messageStr = message.split(':::')[1] as string
    }
    try {
      const data = JSON.parse(messageStr)
      console.log(data)
      if (ws.data.gameSocket == true) {
        switch (ws.data.developer) {
          case 'kickass':
            await this.handleKickassMessage(ws, data as SubscribeMessage)
            break

          case 'netgame':
            await this.handleNetGameMessage(ws, data as NetGameMessage)
            break
          case 'nolimit':
            await this.handleNolimitMessage(ws, data as SubscribeMessage)
            break
        ...
        if (data.gameData !== undefined) {
          this.monitoring?.emit({
            ...
            timestamp: Date.now(),
            socketId: ws.data.socketId,
            data: { event: data.gameData },
          })
        }
        // Validate message
        if (this.validator && ws.data.gameSocket == false) {
      ...
      // Handle the message
      switch (sanitized.event && ws.data.gameSocket == false) {
        case 'subscribe':
   ...
   * Handle client events (whisper)
   */
  private async handleNetGameMessage(ws: ServerWebSocket<WebSocketData>, message: NetGameMessage): Promise<void> {

    // await handleNetGameMessage(ws, message)
    gameplayService.processPHPBetOutcome(ws.data.user, ws.data.gameName, message, null)
  }
  /**
   * Handle WebSocket close
   */
  public handleClose(ws: ServerWebSocket<WebSocketData>, code: number, reason: string): void {
   ...
   */
  public handleDrain(ws: ServerWebSocket<WebSocketData>): void {
