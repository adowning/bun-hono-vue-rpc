...
export interface WebSocketData {
  ...
  socketId: string
  gameSocket: boolean
  developer: 'futex' | 'kickass' | 'nolimit' | 'netgame'
  connectedAt: number
  accessToken: string
...
export interface GameData {
  action: 'BalanceRequest' | 'CheckBrokenGameRequest' | 'AuthRequest' | 'PickBonusItemRequest' | 'APIVersionRequest' | 'EventRequest' | 'InitRequest' | 'SpinRequest'
  msgId: string
  result: boolean
  cmd: 'BalanceRequest' | 'CheckBrokenGameRequest' | 'AuthRequest' | 'PickBonusItemRequest' | 'APIVersionRequest' | 'EventRequest' | 'InitRequest' | 'SpinRequest'
  data: {
    versionMajor: string
    versinoMinor: string
    gameId: string
...
export interface NetGameMessage {
  gameData: GameData
  action: string
  cookie: string
  sessionId: string
...
/**
 * Stub class for SlotSettings.
 * This implementation provides the methods and properties
 * used in your script so the logic can run.
 */

// --- Variable Definitions ---

// Interface for the shape of the postData objecte
