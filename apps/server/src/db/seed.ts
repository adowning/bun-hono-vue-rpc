...
const OPERATOR_NAMES = ['Arcade Ops', 'Web Slots LLC', 'Lucky 7 Gaming']
const OPERATOR_STARTING_BALANCE = 100_000_00 // 1 Million dollars in cents

const SIMULATION_START_DATE = new Date(Date.now() - 90 * 24 * 60 * 60 * 1000) // 90 days ago
const SIMULATION_END_DATE = new Date()
const CHANCE_TO_PLAY_PER_DAY = 0.5
const SESSIONS_PER_DAY = 1
const MIN_BETS_PER_SESSION = 10
const MAX_BETS_PER_SESSION = 150
const MIN_TIME_PER_BET_MS = 3000
const MAX_TIME_PER_BET_MS = 10000
...
// Use the new schema structure for the runtime
...
class UserSimulator {
  ...
  private operatorId: string // <-- NEW: Tracks the user's operator
  private games: schema.Game[]
  ...
  private netCreditFlowToOperator = 0 // <-- NEW: Tracks B2B2C credit flow
  ...
  constructor(
    ...
    this.userId = userId
    this.operatorId = operatorId // <-- NEW
    this.balance = { ...initialBalance }
    this.games = games
    this.createdAt = createdAt
  }

  // --- Financial Event Helpers (Refactored for B2B2C) ---
  ...
  private processInitialDeposit(simDate: Date) {
    ...
    // B2B2C: Credits move FROM operator TO player
    this.netCreditFlowToOperator -= depositAmount

    this.depositLogs.push({
      id: depositId,
      userId: this.userId,
      operatorId: this.operatorId, // <-- Use assigned operatorId
  ...
  private processBet(game: schema.Game, gameSessionId: string, simDate: Date) {
    ...
    // B2B2C: Wager moves FROM player TO operator
    this.netCreditFlowToOperator += wagerAmount
    // B2B2C: Win moves FROM operator TO player
    if (winAmount > 0) {
      this.netCreditFlowToOperator -= winAmount
    }
    ...
    // Create Bet Log (using camelCase)
    this.betLogs.push({
      userId: this.userId,
      operatorId: this.operatorId, // <-- Use assigned operatorId
      status: 'COMPLETED',
  ...
  private processWithdrawal(simDate: Date) {
    ...
    // B2B2C: Credits move FROM player TO operator
    this.netCreditFlowToOperator += withdrawalAmount
  ...
  public simulate() {
    ...
    for (let i = 0; i <= daysSinceJoined; i++) {
      ...
      for (let j = 0; j < SESSIONS_PER_DAY; j++) {
        ...
        for (let k = 0; k < numberOfBets; k++) {
          ...
          currentDate = addMsToDate(currentDate, betTime)
      ...
      // Advance to next day
      currentDate.setDate(currentDate.getDate() + 1)
      currentDate.setHours(randomInt(0, 5), randomInt(0, 59))
    }
    ...
    return {
      ...
      netCreditFlowToOperator: this.netCreditFlowToOperator, // <-- NEW
      operatorId: this.operatorId, // <-- NEW
      depositLogs: this.depositLogs,
      bonusLogs: this.bonusLogs,
      activeBonusInserts: this.activeBonusInserts,
      activeBonusUpdates: this.activeBonuses,
 ...
 */
const main = async () => {
  console.log('--- SEEDING STARTED (B2B2C Multi-Operator Model) ---')
  ...
  // 2. Create N Operators
  console.log(`Creating ${OPERATOR_NAMES.length} operators...`)
  const operatorInserts: NewOperator[] = OPERATOR_NAMES.map((name) => ({
    name: name,
    ownerId: 'system',
    balance: OPERATOR_STARTING_BALANCE
  }))

  const operators = await db.insert(schema.operatorTable).values(operatorInserts).returning()

  const operatorBalanceSheet = new Map<string, number>()
  operators.forEach((op) => operatorBalanceSheet.set(op.id, op.balance))

  console.log(`Created ${operators.length} operators.`)

  // 3. Load and Insert Games (Globally)
  ...
  // Get all games we just inserted
  ...
  for (let i = 0; i < USER_COUNT; i++) {
    ...
    // Assign user to a random operator
    const randomOp = operators[randomInt(0, operators.length - 1)]!

    const user: NewUser = {
      id: faker.string.uuid(),
      authId: faker.string.uuid(),
      operatorId: randomOp.id, // <-- Assign to a real operator
      email,
    ...
    // Pass the user's operatorId to the simulator
    const simulator = new UserSimulator(user.id!, randomOp.id, initialSimState, allGames, createdAt)
  ...
  // 6. Aggregate Game Stats & Operator Balances
  console.log('Aggregating game and operator stats...')

  // Aggregate Game Stats (Global)
  ...
  // Aggregate Operator Balances (B2B2C)
  for (const output of allSimOutputs) {
    const oldBalance = operatorBalanceSheet.get(output.operatorId) || 0
    operatorBalanceSheet.set(output.operatorId, oldBalance + output.netCreditFlowToOperator)
  }

  console.log('--- Operator Final Balances ---')
  operatorBalanceSheet.forEach((balance, opId) => {
    const opName = operators.find((o) => o.id === opId)?.name
    console.log(`${opName}: ${(balance / 100).toFixed(2)}`)
  })
  console.log('---------------------------------')
  ...
  try {
    await db.transaction(async (tx) => {
      ...
      for (const [opId, newBalance] of operatorBalanceSheet.entries()) {
        await tx
          .update(schema.operatorTable)
          .set({
            balance: newBalance,
            updatedAt: new Date()
          })
          .where(sql`${schema.operatorTable.id} = ${opId}`)
      }
    })
  } catch (err) {
