...
<template>
  <div class="page-content !mb-5">
    <ElRow justify="space-between" :gutter="10">
      <ElCol :lg="6" :md="6" :sm="14" :xs="16">
        <ElInput
          ...
          :prefix-icon="Search"
          clearable
          placeholder="Enter the article title to query"
    ...
    <div class="mt-5">
      <div
        class="grid grid-cols-5 gap-2 max-2xl:grid-cols-4 max-xl:grid-cols-3 max-lg:grid-cols-2 max-sm:grid-cols-1"
      >
        <div
          class="group c-p overflow-hidden border border-primary rounded-custom-sm"
        ...
        >
          <div class="relative top-0 left-0 w-full z-20">
            <div class="flex flex-row w-full justify-between">
              ...
              <el-check-tag
                ...
                @change="handleFeaturedToggle(item)"
                @click.stop
              >
                FEATURED
              </el-check-tag>
              ...
              <span
                class="flex border-white/50 border-1 rounded bg-black/50 p-1"
                @click.stop="item.isActive = !item.isActive"
              >
                <ArtSvgIcon
                  :icon="item.isActive ? 'hugeicons:checkmark-circle-03' : 'hugeicons:circle'"
                  :class="
                    item.isActive
          ...
          <div
            ...
            style="transform: scale(0.95); transform-origin: center"
          >
            <ElImage
              class="flex align-center justify-center w-full h-full object-cover bg-[#060e20]"
              ...
              <template #error>
                <div class="flex items-center justify-center w-full h-full bg-gray-100">
                  <img
                    src="/images/logo.png"
                    alt="Fallback"
                    class="w-full h-full object-contain bg-[#060e20]"
                  />
                </div>
              </template>
            </ElImage>
            ...
            <img
              v-if="item.volatility"
              :src="`/images/vol${item.volatility}.png`"
              style="transform: translateX(7px) translateY(8px)"
              class="absolute bottom-0 right-0 w-12 h-15 z-10"
              alt="Volatility"
            />
          </div>
          ...
          <div class="px-2 py-0">
            ...
            <div class="flex-b w-full h-6 mt-0">
              <div class="flex-c justify-start gap-2 flex-row w-full text-g-500">
                ...
                <ElTag type="success" size="small" style="font-weight: bolder; font-size: 14px">
                  GGR:{{
                    formatterUSD.format(Math.floor(item.totalBetAmount - item.totalWonAmount) / 100)
                  }}
                </ElTag>
