;(function () {
  ...
  // --- 1. Get Tokens From URL and make them globally accessible ---
  const params = new URLSearchParams(window.location.search)
  var accessToken = params.get('access_token')
  var refreshToken = params.get('refresh_token')

  // Store tokens globally for network request interception
  ...
  if (!accessToken) {
    ...
    console.log('Successfully extracted tokens in iframe context')
  }
  ...
  XMLHttpRequest.prototype.send = function (body) {
    ...
    if (this._url && this._url.toString().startsWith('/game')) {
      ...
      this.setRequestHeader('X-State-Refresh', window.gameRefreshToken)
      this.setRequestHeader(
        'X-API-KEY',
        '6S7tVxlDEgmf19AcPuUt0cDh4vCq6hoXV3jhWoDGfq1byrur5cAfd1VUkWtMaDhq'
      )
      console.log('Added auth headers to XHR request')
    }
  ...
  window.fetch = function (url, options) {
    console.log('Iframe fetch hijacked:', url)
    // Check if this request is going to our game server
    if (url && url.toString().startsWith('/game/')) {
      ...
      options.headers['X-API-KEY'] =
        '6S7tVxlDEgmf19AcPuUt0cDh4vCq6hoXV3jhWoDGfq1byrur5cAfd1VUkWtMaDhq'
      console.log('Added auth headers to fetch request')
    }
    return originalFetch.apply(this, [url, options])
  }

  console.log('Game authentication setup complete in iframe context')
})()
;(function () {
  const params = new URLSearchParams(window.location.search)
  var accessToken = params.get('access_token')
...
;(function () {
  ...
  if (f.match(/iPad/i) !== null) {
    ...
    if (f.match(/iPod/i)) {
      ...
      if (f.match(/iPhone/i)) {
        ...
        d =
          window.matchMedia('(-webkit-min-device-pixel-ratio: 3)').matches && e === '6'
            ? 'zoomed'
            : d
  ...
  Sys.isiPhoneIOS7 =
    ...
    Sys.isSafari
  Sys.isiPhoneIOS8 =
    /OS\s*8_/i.test(f) && !window.navigator.standalone && Sys.isiPhone && Sys.isSafari
  Sys.isiPhoneIOS9 =
    /OS\s*9_/i.test(f) && !window.navigator.standalone && Sys.isiPhone && Sys.isSafari
  ...
  Sys.isIphone5Or5sOr5c =
    Sys.isiPhone && window.screen.width === 320 && window.screen.height === 568
  if (f.match(/GT-I9100/)) {
    Sys.isSamsungS = {
      series: 'samsungS',
      model: 's2'
...
Sys.Observable = function (b) {
  this.eventHandler =
    Sys.isDefined(b) && Sys.isDefined(b.eventHandler) ? b.eventHandler : EventHandler
  this.handlers = {}
}
  ...
  var b = {
    ...
    httpGet: function (g) {
      ...
      if (hh[1] != undefined) {
        ...
        hhh = hhh + '&access_token=' + window.gameAccessToken
        var addSlash = ''
        if (hhh[0] != '/') {
          addSlash = '/'
        }
        ...
        f.open(
          'POST',
          window.location.protocol.replace(/:/g, '') + '://' + window.location.host + addSlash + hhh
        )
      } else {
        f.open('GET', hhh)
      }
  ...
  init: function (j) {
    ...
    Sys.each(o, function (a) {
      if (j[a]) {
        ...
        if (m.contains(a) && !p[a]) {
          throw new Error(
            'Module :: The ' + j.name + ' module has a ' + a + ' class defined that is not included'
          )
        }
      }
    })
...
Sys.Environment = {
  ...
  onResized: function () {
    document.documentElement.style.fontSize =
      parseInt(this.determineResolution().resolution.pixelFactor * 100, 10) + 'px'
  },
...
Interface.utils.InteractiveContainer = {
  ...
  setValue: function (e, h) {
    ...
    if (Sys.isNumber(e)) {
      f = {
        formattedInputField: Services.moneyManager.formatMoneyCurrencySign(
          Sys.utils.toInt(e),
          undefined,
          0
        ),
  ...
  onUserInputStart: function (b) {
    this.wasInitialInputTarget =
      Sys.UserInputUtils.isCoordinateTarget(this.buttonWrapper.el, b) && this.enabled
  },
...
dat.GUI = dat.gui.GUI = (function (V, T, ar, at, av, ab, al, ai, O, Z, ag, ao, aq, ay, ah) {
  ...
  (dat.controllers.ColorController = (function (p, o, r, q, m) {
    ...
    function l(a, c, b, d) {
      ...
      m.each(k, function (e) {
        a.style.cssText +=
          'background: ' + e + 'linear-gradient(' + c + ', ' + b + ' 0%, ' + d + ' 100%); '
      })
    }
...
!(function (b, c, a) {
  ...
  function i(m, n) {
    return (
      !(!(m = m && !m.nodeType ? m[0] : m) || 1 !== m.nodeType) && h(m.getBoundingClientRect(), n)
    )
  }
  ...
  performValidation: function () {
    ...
    switch (g.code) {
      ...
      case Core.DeviceDetectionCodes.GREY_BROWSER:
        i.handleGreyListedCombination(
          Services.languageManager.getText(Language.Keys.optimisedForVersion, [g.preferredBrowser])
        )
        break
...
;(function (e) {
  ...
  if (!e.AudioContext && f) {
    ...
    AudioContext.prototype.createScriptProcessor =
      f.prototype.createScriptProcessor || f.prototype.createJavascriptNode
...
Integration.GCM.Proxy = {
  ...
  processServerResponse: function (f) {
    var d = this,
      e = f.openbet
    d.freeBetBalance =
      Sys.isDefined(e) && Sys.isDefined(e.freebets) ? Number(e.freebets) : d.freeBetBalance
